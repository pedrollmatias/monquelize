...
module.exports = {
  ...
  async inventoryAdjustment(req, res, next) {
    ...
    try {
      ...
      productHistory.push({ ...req.body, date: Date.now() });

      switch (req.body.movementType) {
        case '100': // Input
          productInventory.currentAmount += Number(req.body.amount);
          break;
        case '200': // Output
          productInventory.currentAmount -= Number(req.body.amount);
          break;
      ...
      const updatedProduct = await product.editFields({ inventory: productInventory, history: productHistory });
